#!/usr/bin/env python3
"""
Enhanced S&OP Data Ingestion for FMCG Supply Chain Planning
Processes comprehensive dataset and adds all missing data elements to Neo4j graph
"""
...
class EnhancedSOPIngestion:
    ...
    def process_enhanced_sop_dataset(self, excel_file_path):
        """Process the enhanced S&OP dataset and create comprehensive graph schema"""
        
        print("🚀 Processing enhanced S&OP dataset...")
        ...
        try:
            ...
            if 'Master Data' in excel_data:
                ...
                self._process_master_data(excel_data['Master Data'])
            
            if 'Customer Data' in excel_data:
                print("👥 Processing Customer Data...")
                self._process_customer_data(excel_data['Customer Data'])
            
            if 'Order Data' in excel_data:
                print("📋 Processing Order Data...")
                self._process_order_data(excel_data['Order Data'])
            
            if 'Manufacturing Capacity' in excel_data:
                print("🏭 Processing Manufacturing Capacity...")
                self._process_manufacturing_data(excel_data['Manufacturing Capacity'])
            
            if 'Promotional Campaigns' in excel_data:
                print("📢 Processing Promotional Campaigns...")
                self._process_promotional_data(excel_data['Promotional Campaigns'])
            
            if 'Regional Data' in excel_data:
                print("🌍 Processing Regional Data...")
                self._process_regional_data(excel_data['Regional Data'])
            
            if 'Demand Plan' in excel_data:
                print("📈 Processing Demand Plan...")
                self._process_demand_plan(excel_data['Demand Plan'])
            
            if 'Supply Plan' in excel_data:
                print("📦 Processing Supply Plan...")
                self._process_supply_plan(excel_data['Supply Plan'])
            
            if 'Inventory Plan' in excel_data:
                print("🏪 Processing Inventory Plan...")
                self._process_inventory_plan(excel_data['Inventory Plan'])
            
            if 'Financial Plan' in excel_data:
                print("💰 Processing Financial Plan...")
                self._process_financial_plan(excel_data['Financial Plan'])
            
            if 'Logistics Plan' in excel_data:
                print("🚚 Processing Logistics Plan...")
                self._process_logistics_plan(excel_data['Logistics Plan'])
            
            print("✅ Enhanced S&OP dataset processing completed successfully!")
            
        except Exception as e:
            print(f"❌ Error processing enhanced S&OP dataset: {str(e)}")
            raise
    
    def _process_master_data(self, master_df):
        """Process enhanced master data with manufacturing and regional information"""
    ...
    def _process_customer_data(self, customer_df):
        """Process customer relationship data"""
    ...
    def _process_order_data(self, order_df):
        """Process order management data"""
        
        print(f"  - Processing {len(order_df)} orders...")
        ...
        for idx, row in order_df.iterrows():
            ...
            try:
                ...
                order_properties = {
                    'order_id': order_id,
                    'customer_id': customer_id,
                    'order_date': str(row.get('order_date', '2024-01-01')),
                    'delivery_date': str(row.get('delivery_date', '2024-01-15')),
                ...
                order_query = """
                MERGE (ord:Order {order_id: $order_id})
                SET ord += $properties
                """
                self.graph.query(order_query, {
                    'order_id': order_id,
                ...
                MATCH (ord:Order {order_id: $order_id})
                MERGE (cust)-[:PLACES]->(ord)
                """
                self.graph.query(customer_order_query, {
                    'customer_id': customer_id,
                    'order_id': order_id
                })
                ...
                if 'items' in row and isinstance(row['items'], str):
                    try:
                        # Handle the case where items might be stored as a string representation
                        items_str = row['items']
                        if items_str and items_str != 'nan':
                            # Try to parse as JSON, but handle potential format issues
                            try:
                                items = json.loads(items_str)
                            except json.JSONDecodeError:
                                # If JSON parsing fails, try to extract basic item info
                                print(f"    - Warning: Could not parse JSON for order {order_id}, skipping items")
                                continue
                            ...
                            for item in items:
                                ...
                                if sku_id:
                                    # Create OrderItem node
                                    item_query = """
                                    MERGE (item:OrderItem {
                                        order_id: $order_id,
                                        sku_id: $sku_id
                                    })
                                    ...
                                    item_relationships_query = """
                                    MATCH (ord:Order {order_id: $order_id})
                                    MATCH (item:OrderItem {order_id: $order_id, sku_id: $sku_id})
                                    MATCH (sku:SKU {sku_id: $sku_id})
                                    MERGE (ord)-[:CONTAINS]->(item)
                                    MERGE (item)-[:REFERENCES]->(sku)
                                    """
                                    self.graph.query(item_relationships_query, {
                                        'order_id': order_id,
                                        'sku_id': sku_id
                                    })
                    except Exception as e:
                        print(f"    - Error processing order items for {order_id}: {str(e)}")
                        continue
        ...
        print(f"  ✅ Order data processing completed")
    
    def _process_manufacturing_data(self, manufacturing_df):
        """Process manufacturing capacity data"""
    ...
    def _process_promotional_data(self, promotional_df):
        """Process promotional campaign data"""
    ...
    def _process_regional_data(self, regional_df):
        """Process regional market data"""
    ...
    def _process_demand_plan(self, demand_df):
        """Process enhanced demand plan with regional and promotional impacts"""
    ...
    def _process_supply_plan(self, supply_df):
        """Process enhanced supply plan with manufacturing constraints"""
    ...
    def _process_inventory_plan(self, inventory_df):
        """Process enhanced inventory plan"""
    ...
    def _process_financial_plan(self, financial_df):
        """Process enhanced financial plan"""
    ...
    def _process_logistics_plan(self, logistics_df):
        """Process enhanced logistics plan"""
    ...
    def create_vector_index(self):
        """Create vector index for enhanced data"""
...
def main():
    """Main function to process enhanced S&OP dataset"""
    
    # Initialize ingestion processor
    processor = EnhancedSOPIngestion()
    ...
    try:
        # Process the dataset
        processor.process_enhanced_sop_dataset(excel_file)
        
        # Create vector index
        processor.create_vector_index()
        
        print("\n🎉 Enhanced S&OP data ingestion completed successfully!")
        ...
        print("  - Order management data with realistic patterns")
        ...
        print(f"❌ Error during enhanced S&OP ingestion: {str(e)}")
        raise
